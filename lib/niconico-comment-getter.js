...
const wrapByPromise = require("./wrap-by-promise.js");
//urlの検索クエリをオブジェクトに展開するやつ
const decodeSearchQuery = require("./decode-search-query.js");
...
class NiconicoCommentGetter {
    ...
    async getComment(id, cb) {
        const opt = {};
        try {
            const isUserVideo = id.indexOf("sm") !== -1
            if (isUserVideo) {
            ...
            let len = thumbInfo.body
                ...
                .match(/[0-9]+:[0-9]{2}/)[0]
                .split(":");
            opt.length = +len[0] + +!!len[1];
            const result = await this._post("http://nmsg.nicovideo.jp/api.json/", {
                body: JSON.stringify(isUserVideo ? paramTemp.userVideo(opt) : paramTemp.channelVideo(opt))
            });
